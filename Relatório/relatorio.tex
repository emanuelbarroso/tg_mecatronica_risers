...
\resumo{resumo}{
A extração de petróleo em águas profundas requer operações bastante complexas. Uma destas operações é a reentrada, na qual um \textit{riser} é conectado da plataforma ao poço de petróleo. O deslocamento do \textit{riser} ocorre a partir da posição da plataforma, e deve ser feito de forma rápida e precisa. Atualmente, o controle de posição é manual, o que significa que a operação como um todo pode ser melhorada. O presente trabalho busca validar experimentos que simulam controladores baseados em controle clássico, utilizando malha aberta e malha fechada, para operar o deslocamento do \textit{riser} de forma automática, com resultados rápidos e precisos, além de oferecer resistência a perturbações causadas pelo oceano, para que a ponta do \textit{riser} seja corretamente conectada ao poço.
...
\resumo{Abstract}{
Petroleum extraction in deep waters requires very complex operations. One of these operations is re-entry, in which a riser is connected from the platform to the petroleum well. The riser's displacement occurs from the platform's position, and must be done quickly and precisely. Presently, the position control is manual, which means that the operation can be upgraded as a whole. The present paper seeks to validate experiments that simulate basic controllers based in classical control, using open loop and closed loop, to operate riser's displacement automatically, with fast and precise results, in addition to offering resistance against disturbances caused by the ocean, so the riser's tip is correctly connected to the well.
